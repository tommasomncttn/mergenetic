...
class Searcher:
    def __init__(self, problem: MergingProblem, algorithm: Algorithm, results_path: str, n_iter: int, seed: int, run_id: str, verbose: bool = True):
        """
    ...
    def search(self) -> pd.DataFrame:
        ...
        result = minimize(self.problem, self.algorithm, ('n_iter', self.n_iter), seed=self.seed, verbose =self.verbose)
        ...
        if len(self.result_X.shape) > 1:
            for x in self.result_X:
                ...
                if isinstance(self.problem.results_df, pd.DataFrame):    
                    ...
                    results_df.to_csv(self.results_path / Path(f"{self.run_id}_test.csv"))
                else:
    ...
    def visualize_results(self) -> None:
        ...
        if not hasattr(self.problem, 'results_df'):
            raise AttributeError("Problem does not have 'results_df' to visualize results.")
