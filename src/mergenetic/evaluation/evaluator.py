...
class BaseEvaluator(ABC):
    ...
    def _validate_dataframe(df: pd.DataFrame) -> None:
        ...
        if "predictions" not in df.columns or "answer" not in df.columns:
            raise ValueError("DataFrame must contain 'predictions' and 'answer' columns.")
...
class LanguageDetector:
    ...
    def __init__(self, langs: list[str], fasttext_model_path: str = f'{PROJECT_ROOT}/models/fasttext/lid.176.bin'):
        """
