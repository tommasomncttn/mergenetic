...
from callbacks import (
    EVAL_MODES,
    ALLOWED_BENCHMARKS,
    yaml_preview_from_ui_single,
    generate_config_and_save_single,
    update_benchmark_visibility,
    start_experiment_single,
    execute_experiment_single,
    refresh_configurations,
    load_config_to_ui_single,
)
...
def create_interface():
    ...
    with gr.Blocks(title="Mergenetic GUI (Single Task)") as demo:
        gr.Markdown("# Mergenetic: Evolutionary Model Merging — Single Task UI")
        gr.Markdown(
            "This simplified UI targets a single task only. Advanced options are tucked into an accordion."
        )
        ...
        # Single-task specific fields (kept visible)
        with gr.Row():
            model_path_single = gr.Textbox(label="Model path", value="model_en")
            task_single = gr.Dropdown(
        ...
        with gr.Accordion("Advanced optimisation & system (optional)", open=False):
            ...
            dtype = gr.Dropdown(["float16", "float32", "bfloat16"], value="float16", label="dtype")
            metric = gr.Textbox(label="Metric (lm-eval)", value="exact_match")

            cfg_dir = gr.Textbox(
                label="Path to store config",
                value=str(create_config_directory()),
            )
        ...
        experiment_status = gr.Textbox(label="Experiment status", value="Not started", interactive=False)
        log_output = gr.TextArea(label="Experiment log", interactive=False, lines=20, autoscroll=True)
        ...
        for c in preview_inputs:
            c.change(yaml_preview_from_ui_single, inputs=preview_inputs, outputs=cfg_preview)

        gen_btn.click(
            fn=generate_config_and_save_single,
            inputs=preview_inputs,
            outputs=[cfg_preview, cfg_path, cfg_generated],
        )

        launch_btn.click(
            fn=start_experiment_single,
            inputs=[cfg_path],
            outputs=[experiment_status, log_output],
        )
        launch_btn.click(
            fn=execute_experiment_single,
            inputs=[cfg_path],
            outputs=log_output,
            show_progress=True,
            queue=True,
        )
        stop_btn.click(fn=stop_experiment, inputs=[], outputs=[experiment_status, log_output])

        # Refresh config list
        refresh_cfg_btn.click(refresh_configurations, inputs=[], outputs=config_dropdown)

        # Load config -> fill UI
        config_dropdown.change(
            fn=load_config_to_ui_single,
            inputs=[
                config_dropdown,
                run_id,
                base_model,
