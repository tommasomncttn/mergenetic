...
def prepare_fasttext():
    ...
    if not os.path.exists(f"{parent_dir}/models/fasttext/lid.176.bin"):
        ...
        with open(f"{parent_dir}/models/fasttext/lid.176.bin", "wb") as f:
            f.write(requests.get("https://dl.fbaipublicfiles.com/fasttext/supervised-models/lid.176.bin").content)
        
        logger.info("fastText model downloaded successfully and saved in models/fasttext/lid.176.bin")
    else:
