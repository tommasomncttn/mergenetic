...
import argparse
import pandas as pd
from mergenetic.optimization.predefined_problems import (MultilingualMergingProblem, 
...
def main(config: Config, no_preeval: bool=False):
    ...
    anchors = {key: datasets[key].sample(random_state=config.seed, n=config.n_samples).index for key in datasets.keys()}
    anchors_weights = {key: [1/config.n_samples]*config.n_samples for key in datasets.keys()}
