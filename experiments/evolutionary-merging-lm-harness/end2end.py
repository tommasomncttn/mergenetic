...
from mergenetic.optimization.predefined_problems import CrossLingualMathProblem, ConfigPE
from mergenetic.evaluation.utils import (
    evaluate_models_lm_harness,
    retrieve_thetas
)
...
def main(config: ConfigLmEval):
    ...
    anchors = np.random.choice(range(len(task.dataset["test"])), config.n_samples, replace=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--seed', type=int, help='The seed to use for the experiment.', default=None)
    ...
    parser.add_argument('--n_samples', type=int, help='The number of samples to use for the experiment.', default=None)
