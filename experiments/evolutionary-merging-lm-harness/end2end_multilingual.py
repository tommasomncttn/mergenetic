...
def main(config: ConfigLmEval):
    ...
    for lang in config.langs:
        ...
        anchors[lang] = np.random.choice(range(len(task.dataset["test"])), config.n_samples, replace=False)
    ...
    problem = MultilingualMergingProblem(
                                    ...
                                    config_pe=conf_pe,
                                    n_var=n_var,
                                    n_obj=len(tasks['search'])-1, # -1 because the last task is only used to evaluate the base model
                                    n_eq_constr=0,
                                    n_ieq_constr=0,
