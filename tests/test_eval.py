...
class TestExtractNumbers(unittest.TestCase):
    def test_only_last_number_true_multiple_numbers(self):
        self.assertEqual(extract_numbers("Sentence with 123 and 45.67 then 890."), "890")
    ...
    def test_only_last_number_true_numbers_with_letters(self):
        self.assertEqual(extract_numbers("Number 123abc should be 456"), "456") # 123abc is not matched due to (?!\w)
    ...
    def test_only_last_number_false_multiple_numbers(self):
        self.assertEqual(extract_numbers("Sentence with 123 and 45.67 then 890.", only_last_number=False), ["123", "45.67", "890"])
    ...
    def test_only_last_number_false_single_number(self):
        ...
        self.assertEqual(extract_numbers("Only 1 number 123 here.", only_last_number=False), ["1", "123"])
    ...
    def test_only_last_number_false_float_and_int(self):
        self.assertEqual(extract_numbers("Value 1.23 and 456.", only_last_number=False), ["1.23", "456"])
...
class TestMCEvaluator(unittest.TestCase):
    ...
    def setUp(self):
        self.sample_df = pd.DataFrame({
            'predictions': ["Answer is 7", "The correct one is 3.14", "No idea, maybe 0"],
            'answer': ["7", "3.14", "1"]
        })
        self.lang_id_it = "it"
    ...
    @patch('mergenetic.evaluation.math_language.LanguageDetector', side_effect=ValueError("Test Detector Error"))
    def test_init_language_detector_fails(self, mock_lang_detector_class, mock_logger):
        evaluator = MCEvaluator(language_id=self.lang_id_it)
        
        self.assertIsNone(evaluator.lang_detector)
        mock_logger.warning.assert_called_once_with("Language detection disabled: Test Detector Error")

    @patch.object(MCEvaluator, '_validate_dataframe') # Patching the method on the class
    def test_get_correctness_no_language_id(self, mock_validate_df):
        evaluator = MCEvaluator(language_id=None)
        correctness = evaluator.get_correctness(self.sample_df.copy()) # Use copy to avoid modifying original
        ...
        pd.testing.assert_series_equal(evaluator.data["filtered_answer"], expected_filtered_answer, check_names=False)
        pd.testing.assert_series_equal(correctness, expected_correctness, check_names=False)

    @patch.object(MCEvaluator, '_validate_dataframe')
    @patch('mergenetic.evaluation.math_language.LanguageDetector')
    def test_get_correctness_with_language_id(self, mock_lang_detector_class, mock_validate_df):
        mock_detector_instance = MagicMock()
        mock_detector_instance._get_language.side_effect = ["__label__it", "__label__it", "UNK"] # Mocked lang detection
        mock_lang_detector_class.return_value = mock_detector_instance

        evaluator = MCEvaluator(language_id=self.lang_id_it)
        self.assertIsNotNone(evaluator.lang_detector)
        self.assertEqual(evaluator.lang_detector, mock_detector_instance) # Verify mock is set


        correctness = evaluator.get_correctness(self.sample_df.copy())
        
        mock_validate_df.assert_called_once()
        self.assertEqual(mock_detector_instance._get_language.call_count, len(self.sample_df))

        expected_language = pd.Series(["__label__it", "__label__it", "UNK"], name="language")
        expected_filtered_answer = pd.Series(["7", "3.14", "0"], name="filtered_answer")
        expected_is_lang_correct = pd.Series([True, True, True], name="is_language_correct") # UNK is also correct
        expected_is_answer_correct = pd.Series([True, True, False], name="is_answer_correct")
        expected_final_correctness = pd.Series([True, True, False], name="correctness") # True & True, True & True, True & False

        pd.testing.assert_series_equal(evaluator.data["language"], expected_language, check_names=False)
        pd.testing.assert_series_equal(evaluator.data["filtered_answer"], expected_filtered_answer, check_names=False)
        pd.testing.assert_series_equal(evaluator.data["is_language_correct"], expected_is_lang_correct, check_names=False)
        pd.testing.assert_series_equal(evaluator.data["is_answer_correct"], expected_is_answer_correct, check_names=False)
        pd.testing.assert_series_equal(correctness, expected_final_correctness, check_names=False)
...
class TestFGMathEvaluator(unittest.TestCase):
    ...
    def setUp(self):
        self.sample_df = pd.DataFrame({
            'predictions': ["The final answer is 123.", "I think it's 456.", "Result: 7890"],
            'answer': ["123", "450", "7890"]
        })
        self.lang_id_en = "en"
    ...
    def test_init_with_language_id_none(self):
        ...
        self.assertFalse(hasattr(evaluator, 'lang_detector') and evaluator.lang_detector is not None)


    def test_extract_answer_static_method(self):
        self.assertEqual(FGMathEvaluator._extract_answer("Answer is 123 then 456."), "456")
        ...
        self.assertEqual(FGMathEvaluator._extract_answer("Number 12.34 should be 56"), "56") # Extracts last sequence of digits
    ...
    def test_get_correctness_no_language_id(self, mock_validate_df):
        ...
        mock_validate_df.assert_called_once()
        expected_filtered = pd.Series(["123", "456", "7890"], name="predictions_filtered")
        expected_correctness = pd.Series([True, False, True], name="correctness")

        pd.testing.assert_series_equal(evaluator.data["predictions_filtered"], expected_filtered, check_names=False)
        pd.testing.assert_series_equal(correctness, expected_correctness, check_names=False)
    ...
    def test_get_correctness_with_language_id(self, mock_lang_detector_class, mock_validate_df):
        mock_detector_instance = MagicMock()
        mock_detector_instance._get_language.side_effect = ["__label__en", "UNK", "__label__fr"] # fr is wrong
        mock_lang_detector_class.return_value = mock_detector_instance
        ...
        lang_correctness = pd.Series([True, True, False]) # Language correctness (en or UNK is True, fr is False)
