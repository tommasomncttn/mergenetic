...
MODEL_PATHS_MULTI_STR = ["OpenLLM-Ro/RoMistral-7b-Instruct", "DeepMount00/Mistral-Ita-7b"]
...
class TestTiesMerger(BaseMergerTest):
    ...
    def test_initialization(self):
        ...
        self.assertEqual(self.merger.model_paths, [Path(p) for p in MODEL_PATHS_MULTI_STR])
        self.assertEqual(self.merger.dtype, DTYPE)
        # Base class attributes
        self.assertEqual(self.merger.path_to_store_yaml, self.expected_yaml_file_path)
        self.assertEqual(self.merger.path_to_store_merged_model, self.expected_merged_model_path)
    ...
    def test_create_individual_configuration(self, mock_logger, mock_file_open, mock_mkdir, mock_yaml_dump):
        weights_and_densities = [0.7, 0.3, 0.9, 0.6] # w1, w2, d1, d2
        num_models_to_merge = len(MODEL_PATHS_MULTI_STR)

        returned_path = self.merger.create_individual_configuration(weights_and_densities)
        ...
        self.assertEqual(config_dumped['models'][1]['parameters']['weight'], weights_and_densities[0])
        self.assertEqual(config_dumped['models'][1]['parameters']['density'], weights_and_densities[num_models_to_merge])

        mock_logger.info.assert_called_with(f"Configuration file created at {self.expected_yaml_file_path}")
...
class TestTiesDareMerger(BaseMergerTest):
    ...
    @patch('builtins.open', new_callable=mock_open)
    def test_create_individual_configuration(self, mock_file_open, mock_mkdir, mock_yaml_dump):
...
class TestDareTaskArithmeticMerger(BaseMergerTest):
    ...
    def test_create_individual_configuration(self, mock_file_open, mock_mkdir, mock_yaml_dump):
        ...
        args, _ = mock_yaml_dump.call_args
        config_dumped = args[0]
        self.assertEqual(config_dumped['merge_method'], 'dare_linear') # Code uses dare_linear
        self.assertEqual(config_dumped['base_model'], BASE_MODEL_PATH_STR)
...
class TestMergerBaseFunctionality(BaseMergerTest):
    def setUp(self):
        ...
        class DummyMerger(Merger): # Dummy concrete class for testing base Merger
            def create_individual_configuration(self, *args, **kwargs): return Path("dummy.yaml")
    ...
    def test_check_and_delete_yaml_not_exists(self, mock_logger):
        self.assertFalse(self.expected_yaml_file_path.exists()) # Ensure it doesn't exist
        self.merger_instance.check_and_delete_yaml()
        mock_logger.info.assert_called_with(f"No file found at: {self.expected_yaml_file_path}")
    ...
    def test_delete_merged_model_local_exists(self, mock_rmtree, mock_logger):
        ...
        self.assertTrue(self.expected_merged_model_path.exists(), "Test setup: Directory should exist")
        ...
        mock_logger.info.assert_called_with(f"Deleted folder and all contents: {self.expected_merged_model_path}")
    ...
    def test_delete_merged_model_local_not_exists(self, mock_rmtree, mock_logger):
        ...
        if self.expected_merged_model_path.exists():
             shutil.rmtree(self.expected_merged_model_path) # Explicitly remove if it exists
        self.assertFalse(self.expected_merged_model_path.exists(), "Test setup: Directory should not exist")

        self.merger_instance._delete_merged_model_local() # Call the method under test

        mock_rmtree.assert_not_called()
        mock_logger.info.assert_called_with(f"The folder does not exist: {self.expected_merged_model_path}")
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch.object(Merger, '_delete_merged_model_local') # Patching on the base class itself
    @patch('mergenetic.merging.merger.clean_gpu')
    @patch('mergenetic.merging.merger.torch.cuda.is_available', return_value=True)
    @patch('mergenetic.merging.merger.logger')
    def test_merge_model_from_configuration(self, mock_base_logger, mock_cuda_available, 
                                           mock_clean_gpu, mock_delete_model, mock_file_open_read, 
                                           mock_yaml_safe_load, mock_model_validate, mock_run_merge):
        ...
        result_path = self.merger_instance.merge_model_from_configuration(self.expected_yaml_file_path)
        ...
        mock_file_open_read.assert_called_once_with(self.expected_yaml_file_path, "r", encoding="utf-8")
