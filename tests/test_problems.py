...
class TestConfigClasses(unittest.TestCase):
    ...
    def test_config_lm_eval_multiobjective_pe_post_init_validation(self):
        with self.assertRaisesRegex(ValueError, "Tasks should be a list of task names."):
            ConfigLmEvalMultiObjectivePE(tasks="not_a_list", correct_metric="acc", sample_ids={})
        
        with self.assertRaisesRegex(ValueError, "Sample IDs should be a dictionary of lists of integers."):
            ConfigLmEvalMultiObjectivePE(tasks=["t1"], correct_metric="acc", sample_ids="not_a_dict")

        with self.assertRaisesRegex(ValueError, "Correctness metric should be a string."):
            ConfigLmEvalMultiObjectivePE(tasks=["t1"], correct_metric=123, sample_ids={"t1":[1]})
        
        with self.assertRaisesRegex(ValueError, "Additional templates folder should be a string or None."):
            ConfigLmEvalMultiObjectivePE(tasks=["t1"], correct_metric="acc", sample_ids={"t1":[1]}, additional_templates_folder=123)
...
class TestCrossLingualMathProblem(unittest.TestCase):
    def setUp(self):
        ...
        self.lang_id = "it"
        self.conf_pe = ConfigPE(thetas=[[0.1]], sample_ids=[0], weights=[1.0], correct_metric="exact_match", mode="mean")
        self.lm_eval_tasks = {"search": {"it": "arc_it_search"}, "test": {"it": "arc_it_test"}}
    ...
    def _create_problem(self, use_lm_eval=False, **kwargs):
        ...
        problem.load_model = MagicMock(return_value=("mock_model", "mock_tokenizer") if not use_lm_eval else "mock_vllm_model")
        problem._from_array_to_genotype = MagicMock(return_value=Path("/fake/merged_model"))
        return problem
    ...
    def test_metrics_4_genotype_no_lm_eval(self, MockPerformanceEstimator, MockFGMathEvaluator, mock_get_preds):
        problem = self._create_problem(use_lm_eval=False)
        mock_model, mock_tokenizer = "fake_model", "fake_tokenizer"
        
        mock_get_preds.return_value = pd.Series(["prompt_text_ans1"]) # predictions after removing prompt
        ...
        pd.testing.assert_series_equal(called_args[0], correctness_series_expected, check_dtype=False)
    ...
    @patch('mergenetic.optimization.predefined_problems.PerformanceEstimator')
    def test_metrics_4_genotype_with_lm_eval(self, MockPerformanceEstimator, MockLmHarnessEvaluator):
        problem = self._create_problem(use_lm_eval=True)
    ...
    def test_test_method_with_genotype(self):
        ...
        problem.metrics_4_genotype.assert_called_once_with("mock_model", "mock_tokenizer")
...
class TestMultilingualMergingProblem(unittest.TestCase):
    def setUp(self):
        ...
        self.search_dfs = {"en": pd.DataFrame({'prompt': ['q_en']}), "fr": pd.DataFrame({'prompt': ['q_fr']})}
        self.test_dfs = {"en": pd.DataFrame({'prompt': ['t_en']}), "fr": pd.DataFrame({'prompt': ['t_fr']})}
        self.conf_pe = ConfigMultiLingualPE(
            sample_ids={"en": [0], "fr": [0]},
            weights={"en": [1.0], "fr": [1.0]},
            thetas=[[0.1]], bench="test_bench", mode="mean", correct_metric="acc"
        )
        self.lm_eval_tasks = {
    ...
    def _create_problem(self, use_lm_eval=False, **kwargs):
        ...
        problem = MultilingualMergingProblem(
            ...
            lm_eval_tasks=lm_tasks,
            n_var=3, n_obj=2, # n_obj must match number of languages/objectives
            **kwargs
        )
        problem.load_model = MagicMock(return_value=("mock_model", "mock_tokenizer") if not use_lm_eval else "mock_vllm_model")
        problem._from_array_to_genotype = MagicMock(return_value=Path("/fake/merged_model"))
    ...
    @patch('mergenetic.optimization.predefined_problems.PerformanceEstimator')
    def test_metrics_4_genotype_no_lm_eval(self, MockPerfEst, MockMultiMCEval, mock_get_preds):
        problem = self._create_problem(use_lm_eval=False)
        ...
        mock_multi_eval_inst.get_correctness.return_value = {"en": pd.Series([True]), "fr": pd.Series([False])}
        ...
        MockMultiMCEval.assert_called_once_with(language_ids=["en", "fr"], validate_lang=True)
    ...
    def test_metrics_4_genotype_with_lm_eval(self, MockPerfEst, MockLmHarness):
        ...
        mock_lm_h_inst.evaluate.side_effect = [pd.Series([True]), pd.Series([False, True])] # en, fr correctness
...
class TestLmEvalMultiObjectiveProblem(unittest.TestCase):
    ...
    def _create_problem(self, **kwargs):
        problem = LmEvalMultiObjectiveProblem(
            config=self.config,
            merger=self.mock_merger,
            n_var=3, n_obj=2, # n_obj must match number of tasks
            **kwargs
        )
    ...
    def test_metrics_4_genotype(self, MockPerfEst, MockLmHarness):
        ...
        mock_lm_h_inst = MockLmHarness.return_value
        mock_lm_h_inst.evaluate.side_effect = [pd.Series([True]), pd.Series([False, True])] # task_a, task_b
        ...
        MockLmHarness.assert_any_call(
            task_name="task_a", sample_ids=self.config.sample_ids["task_a"], 
            correctness_metric=self.config.correct_metric, is_test=False, 
            additional_templates_folder=None, batch_size=problem.eval_batch_size
        )
        MockLmHarness.assert_any_call(
            task_name="task_b", sample_ids=self.config.sample_ids["task_b"], 
            correctness_metric=self.config.correct_metric, is_test=False, 
            additional_templates_folder=None, batch_size=problem.eval_batch_size
        )
        ...
        est_params_calls = [call_args[0][0] for call_args in MockPerfEst.call_args_list] # Get the est_params argument
        self.assertEqual(est_params_calls[0].sample_ids, self.config.sample_ids["task_a"])
        self.assertEqual(est_params_calls[1].sample_ids, self.config.sample_ids["task_b"])
    ...
    def test_test_method_with_genotype(self):
        problem = self._create_problem()
        problem.metrics_4_genotype = MagicMock(return_value=([-0.9, -0.85], "Test desc"))
        ...
        self.assertTrue(problem.test_mode)
        problem._from_array_to_genotype.assert_called_once_with(genotype)
        problem.load_model.assert_called_once_with(Path("/fake/merged_model")) # Path from _from_array_to_genotype mock
        problem.metrics_4_genotype.assert_called_once_with("mock_vllm_model")
