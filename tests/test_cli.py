...
class BaseCLITest(unittest.TestCase):
    def setUp(self):
        ...
        (self.mock_project_root / "experiments" / "evolutionary-merging-lm-harness").mkdir(parents=True, exist_ok=True)
        (self.mock_project_root / "experiments" / "evolutionary-merging").mkdir(parents=True, exist_ok=True)
        (self.mock_project_root / "configs").mkdir(parents=True, exist_ok=True)

        self.project_root_patcher = patch.object(cli_module, 'PROJECT_ROOT', self.mock_project_root)
        ...
        self.mock_in_memory_history_class.return_value = self.mock_in_memory_history_instance

        if hasattr(cli_module, 'input_history'):
             self.input_history_direct_patcher = patch.object(cli_module, 'input_history', self.mock_in_memory_history_instance)
...
class TestCLIHelpers(BaseCLITest):
    def test_input_with_default(self):
        ...
        self.mock_prompt.assert_called_with("Enter value [default_value]: ", history=self.mock_in_memory_history_instance)
        ...
        self.mock_prompt.assert_called_with("Enter specific value: ", history=self.mock_in_memory_history_instance)

    def test_yes_no_input(self):
        self.mock_prompt.return_value = "yes"
        self.assertTrue(cli_module.yes_no_input("Question?", default="no"))
        self.mock_prompt.assert_called_with("Question? [y/N]: ", history=self.mock_in_memory_history_instance)
        ...
        self.mock_logger.warning.assert_called_with("Please respond with 'yes' or 'no' (or 'y' or 'n').")
...
class TestCLIMainFlow(BaseCLITest):
    ...
    def tearDown(self):
        if hasattr(self, 'open_patcher'): self.open_patcher.stop()
        if hasattr(self, 'yaml_dump_patcher'): self.yaml_dump_patcher.stop()
        if hasattr(self, 'subprocess_run_patcher'): self.subprocess_run_patcher.stop()
        if hasattr(self, 'signal_patcher'): self.signal_patcher.stop()
        super().tearDown()
    ...
    def test_main_flow_args_provided_single_lm_eval_no_run(self):
        ...
        with patch.object(cli_module, 'parse_arguments', return_value=sim_args):
            user_inputs = [
                "test_run_01", "base/model", "cpu", "float32", "MULTIPLE_CHOICE",
                "5", "5", "50", "16", "123",
                str(self.mock_project_root / "merged_models_output"),
                "en", "model/en", "task_en", "acc_norm", "mmlu", "mpirt", "custom_lm_tasks",
                "no"
            ]
            ...
            self.assertEqual(dumped_config['additional_templates_folder'], "custom_lm_tasks")
    ...
    def test_main_flow_interactive_multi_custom_run_success(self):
        ...
        with patch.object(cli_module, 'parse_arguments', return_value=sim_args):
            user_inputs = [
                "no", "yes", 
                "multi_custom_run", "another/base", "cuda:0", "bfloat16", "FG_MATH", "20", "30", "100", "8", "2024",
                str(self.mock_project_root / "custom_merged_output"),
                "2", 
                "de", "model/de", "data/de.csv",
                "es", "model/es", "data/es.csv",
                "custom_bench", "weighted",
                "yes" 
            ]
            self._setup_common_mocks_for_main(user_inputs)
            self.mock_subprocess_run.return_value = MagicMock(check_returncode=lambda: None)
            cli_module.main()
            self.mock_file_open.assert_called_once_with(
            ...
            self.mock_logger.info.assert_any_call("✅ Experiment completed successfully!")
            
    def test_main_flow_run_script_fails(self):
        sim_args = argparse.Namespace(eval_method='custom', merge_type='single')
        with patch.object(cli_module, 'parse_arguments', return_value=sim_args):
            user_inputs = [
                "fail_run", "base/model", "cpu", "float16", "FG_MATH", "2", "2", "2", "2", "1",
                str(self.mock_project_root / "merged_output_fail"),
                "fr", "model/fr", "data/fr.csv", "bench_fr", "mean",
                "yes"
            ]
            self._setup_common_mocks_for_main(user_inputs)
            ...
            for call_args_tuple in self.mock_logger.error.call_args_list:
                ...
                if args and isinstance(args[0], str):
                    if not kwargs.get('exc_info', False):
                        ...
                        break
            self.assertTrue(error_call_found_correctly, "logger.error was not called appropriately")
    ...
    def test_handle_keyboard_interrupt_during_subprocess(self):
        ...
        with patch.object(cli_module, 'parse_arguments', return_value=sim_args):
            user_inputs = [
                "kb_interrupt_run", "base/model", "cpu", "float16", "FG_MATH", "2", "2", "2", "2", "1",
                str(self.mock_project_root / "merged_interrupt"),
                "it", "model/it", "data/it.csv", "bench_it", "mean",
                "yes"
            ]
            self._setup_common_mocks_for_main(user_inputs)
            ...
            self.mock_subprocess_run.assert_called_once()
            self.mock_logger.warning.assert_any_call("⚠️ Experiment interrupted by user (CTRL+C)")
